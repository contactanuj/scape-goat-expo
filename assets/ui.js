...
(function () {
  ...
  function onStep(btn) {
    ...
    if (path === 'playerCount') { setPlayerCount(next); render(); return; }
  ...
  // Change the player count while PRESERVING every other setting (names, bot/human choices,
  // deck, scoring, frame mode, etc.). Only the count-dependent values are re-derived so the
  // setup always respects the player count: per-seat arrays are resized, the 6-player-only
  // interrupt is cleared off 6, hand size is kept >= the feasible minimum, and the flip
  // threshold stays <= prep tokens. Nothing gets stuck or silently wiped.
  function setPlayerCount(n) {
    draft.playerCount = Math.max(3, Math.min(8, n));
    resizeNames(draft);
    var fmin = feasibleMinHand(draft);
    if (draft.handSize < fmin) draft.handSize = fmin;
    if (draft.flipThreshold > draft.prepTokens) draft.flipThreshold = draft.prepTokens;
  }
